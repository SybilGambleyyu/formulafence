...
numeric-error, name-error, null-intersection, dynamic-array-spill, and
value-error results,
explicit-broken-reference, and saved broken-reference-result signals. Together
these produce thirty-five reviewable findings:
...
- `FF105` (high): an infix division expression uses a direct literal zero
  divisor.
...
- `FF110` (high): a native `SUM` call uses direct static ranges that overlap,
  so at least one cell is included more than once.
...
- `FF115` (high): a formula's saved result is a null-intersection error.
- `FF116` (high): a dynamic-array formula's saved result is a spill error.
...
results, and saved numeric-error, name-error, null-intersection,
dynamic-array-spill, value-error, and broken-reference results.
...
with `@`) with exactly two nonempty arguments, one direct static internal A1
cell/range or whole-column array, and a direct signed decimal rank literal. It
reports a negative or zero rank, or a positive rank larger than the array's
rectangular cell capacity. Capacity is an upper bound on numeric data points,
so the rule proves that overage without reading cell values or calculating a
formula. Names, Tables, external or 3-D references, full rows, unions,
computed, dynamic, spill, implicit-intersection, decimal/scientific,
...
stay quiet. Its evidence retains only the affected location and aggregate
direct-zero-divisor count—never a formula, numerator, literal spelling, or
...
cell/range reference. FormulaFence resolves every referenced sheet against the
workbook, then reports only a same-sheet pair of rectangles that intersects, so
at least one cell is included by more than one argument. It does not inspect
cell values, calculate a formula, or decide whether that inclusion was
intentional. Names, Tables, external/3-D, whole-column, or full-row references, unions,
computed/dynamic/spill or argument-level implicit-intersection forms, malformed or
explicit-broken-reference formulas, array territory, unknown sheets, and
arbitrary namespaces remain quiet. Its evidence retains only the affected
location and aggregate overlapping-pair and overlapping-call counts—never a
formula, range spelling, value, or source-sheet identity.
`FF111` accepts only native `AGGREGATE` (optionally with `@`) and its exact
OOXML `_xlfn.AGGREGATE` serialization, with three through 255 nonempty
arguments. It inspects only direct signed decimal integer literals in the
function-number and option positions, reports a function number outside 1–19
or an option outside 0–7, and reports a direct 14–19 function number with only
the first reference. It does not inspect references, values, or results, and
does not calculate a formula. Computed, reference, decimal/scientific, array,
malformed, explicit-broken-reference, array-territory, and arbitrary namespace
forms remain quiet. Its evidence retains only the affected location and
aggregate error-class counts—never a formula, literal value, or reference.
`FF112` accepts only native `MOD` (optionally with `@`) with exactly two
nonempty arguments and a direct signed decimal integer zero divisor. It does
not evaluate the number argument or inspect any value. Computed, reference,
...
encoding for `FF116`, because direct syntax alone cannot prove that a source
...
cache records, and a location already covered by direct-literal `FF105` or
`FF112` stay
quiet. Its evidence retains only the affected location and a saved-result scope.
...
formula, inspect its formula text, retain its cached value, diagnose whether a
name, function, or add-in caused the result, or infer that the current result
...
by direct-static `FF093` or `FF116` stay quiet. Its evidence retains only the
affected location and a saved-result scope.
`FF115` accepts only a well-formed stored formula-result cache whose exact
private error classification is a null-intersection error. It does not
calculate the formula, inspect its formula text, retain its cached value,
diagnose an error cause, or infer that the current result is unchanged. Other
saved error kinds and missing or malformed cache records stay quiet. Its
evidence retains only the affected location and a saved-result scope.
...
relationship and content-type declaration. It does not calculate the formula,
inspect its formula text, retain a cache value or metadata record, diagnose a
blocker or other spill cause, or infer that the current result is unchanged.
Partial, malformed, or ordinary value-error records stay quiet for this rule
and remain eligible for `FF109`. Its evidence retains only the affected
...
error; `FF108` separately accepts an exact saved name error; `FF109`
separately accepts an exact saved value error; and `FF115` separately accepts
an exact saved null-intersection error; `FF116` separately accepts a verified
saved dynamic-array spill encoding. Missing or malformed cache
...
`FF093` or `FF116`. All seven are high-severity records of the last saved
display state, not proof of a formula's current result. The lint does not
calculate formulas and fails closed if the metadata needed for a saved spill
