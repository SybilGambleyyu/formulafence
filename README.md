...
python -m pip install https://github.com/SybilGambleyyu/formulafence/releases/download/v0.196.0/formulafence-0.196.0-py3-none-any.whl
...
conditional-aggregate and `SUMPRODUCT` range-shape, explicit-broken-reference,
...
these produce thirteen reviewable findings:
...
`SUMPRODUCT` range-shape mismatches, and saved broken-reference results.
...
counts, and `SUMPRODUCT` mismatch counts—never formula text, cached values,
ignored-error target ranges, direct conditional-aggregate or `SUMPRODUCT` range
spellings, or Table master formulas. `FF089` accepts
...
| Formula lint | Conservative blank/error, manual-value, text-marker, and formula-outlier candidates inside a single workbook's copied blocks; narrowly scoped simple aggregate ranges that stop before a contiguous numeric gap; direct static conditional-aggregate range-shape mismatches for `SUMIFS`, `COUNTIFS`, `AVERAGEIFS`, `MAXIFS`, and `MINIFS`, plus direct static `SUMPRODUCT` array-shape mismatches; explicit direct unlocks on protected formula cells; and formula workbooks explicitly saved with incomplete manual calculation; copied-pattern findings require three local matching peers |
