...
def _index_literal_position_candidates(
    snapshot: WorkbookSnapshot,
    array_ranges_by_sheet: dict[str, tuple[ArrayFormulaRange, ...]],
    *,
    existing_finding_count: int,
    ...
    """Return cells with provable literal ``INDEX`` position errors.

    The formula helper accepts only native ``INDEX`` calls (optionally preceded
    by ``@``), two or three nonempty arguments, a direct bounded internal A1
    array, and direct bare nonnegative decimal row and optional column
    literals. This layer adds workbook context: ordinary inspectable formula
    cells only, no array territory, and no explicit broken-reference operand.
    Findings retain only aggregate counts, never formula text, position values,
    array range spellings, or source sheet identity.
    """
    candidates: list[tuple[CellKey, int]] = []
    for location in sorted(snapshot.cells, key=_location_sort_key):
        if (
...
def _text_literal_argument_candidates(
    snapshot: WorkbookSnapshot,
    array_ranges_by_sheet: dict[str, tuple[ArrayFormulaRange, ...]],
    *,
    existing_finding_count: int,
    ...
    """Return cells with impossible direct literal text-function arguments.

    The formula helper accepts only native ``LEFT``, ``RIGHT``, ``MID``,
    ``FIND``, and ``SEARCH`` calls with valid arity and direct signed integer
    position/count arguments. This layer adds workbook context: ordinary
    inspectable formula cells only, no array territory, and no explicit
    broken-reference operand. Findings retain only an aggregate count, never
    formula text, literal values, or source sheet identity.
    """
    ...
    for location in sorted(snapshot.cells, key=_location_sort_key):
        ...
        if formula is None:
            continue
        mismatch_count = text_literal_argument_mismatch_count(formula)
        if not mismatch_count:
            continue
        if (
            existing_finding_count + len(candidates)
...
def _direct_zero_divisor_candidates(
    snapshot: WorkbookSnapshot,
    array_ranges_by_sheet: dict[str, tuple[ArrayFormulaRange, ...]],
    *,
    existing_finding_count: int,
    ...
    """Return cells with a syntactically direct literal zero divisor.

    The formula helper accepts only an infix division with an immediate direct
    signed integer zero operand. It does not calculate a numerator or inspect
    any value; parenthesized, powered, percent, computed, reference, decimal,
    scientific, malformed, or repeated-sign divisor forms remain outside the
    contract. This layer limits results to ordinary inspectable formula cells
    outside array territory and retains only an aggregate count.
    """
    candidates: list[tuple[CellKey, int]] = []
    for location in sorted(snapshot.cells, key=_location_sort_key):
        if (
        ...
        if formula is None:
            continue
        mismatch_count = direct_zero_divisor_count(formula)
        if not mismatch_count:
            continue
        if (
            existing_finding_count + len(candidates)
...
def _saved_name_error_result_locations(
    snapshot: WorkbookSnapshot,
) -> tuple[CellKey, ...]:
    """Return locations with a valid saved name-error formula result.
...
def lint_snapshot(
    ...
    *,
    ...
    numeric-error, or name-error results. It never evaluates formulas, and
    rejects incomplete array metadata before claiming ordinary-cell coverage.
    """
    ...
    index_candidates = _index_literal_position_candidates(
        snapshot,
        array_ranges_by_sheet,
        existing_finding_count=(
            len(conditional_aggregate_candidates)
    ...
    text_literal_argument_candidates = _text_literal_argument_candidates(
        snapshot,
        array_ranges_by_sheet,
        existing_finding_count=(
            len(conditional_aggregate_candidates)
    ...
    direct_zero_divisor_candidates = _direct_zero_divisor_candidates(
        snapshot,
        array_ranges_by_sheet,
        existing_finding_count=(
            len(conditional_aggregate_candidates)
    ...
    structural_formula_locations.update(
        location for location, _ in text_literal_argument_candidates
    )
    structural_formula_locations.update(
    ...
    for location, index_call_count in index_candidates:
        if len(findings) >= max_formula_pattern_findings:
            raise FormulaFenceError(
                "Formula lint exceeds "
                f"max_formula_pattern_findings={max_formula_pattern_findings}."
    ...
    for location, invalid_literal_argument_count in text_literal_argument_candidates:
        if len(findings) >= max_formula_pattern_findings:
            raise FormulaFenceError(
                "Formula lint exceeds "
                f"max_formula_pattern_findings={max_formula_pattern_findings}."
        ...
        findings.append(
            Finding(
                ...
                location=location,
                details={
                    "invalid_literal_argument_count": invalid_literal_argument_count,
                    "evidence_scope": "text_direct_signed_integer_position_count",
                },
            )
        )

    for location, direct_zero_divisor_expression_count in direct_zero_divisor_candidates:
        if len(findings) >= max_formula_pattern_findings:
            raise FormulaFenceError(
                "Formula lint exceeds "
                f"max_formula_pattern_findings={max_formula_pattern_findings}."
        ...
        findings.append(
            Finding(
                rule_id="FF105",
                severity="high",
                message="A division expression uses a direct literal zero divisor.",
                location=location,
                details={
    ...
    for location in _saved_name_error_result_locations(snapshot):
        if len(findings) >= max_formula_pattern_findings:
            raise FormulaFenceError(
                "Formula lint exceeds "
                f"max_formula_pattern_findings={max_formula_pattern_findings}."
