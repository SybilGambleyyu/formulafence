...
_RULE_FIELDS = {
    ...
    "no_table_style_control_changes",
...
class Policy:
    ...
    no_table_style_control_changes: bool = False
    no_shared_workbook_revision_changes: bool = False
    ...
    no_cell_border_changes: bool = False
...
def parse_policy(data: object) -> Policy:
    ...
    return Policy(
        ...
        no_table_style_control_changes=_boolean_rule(
            rules, "no_table_style_control_changes"
        ),
        no_shared_workbook_revision_changes=_boolean_rule(
            rules, "no_shared_workbook_revision_changes"
        ),
        ...
        no_cell_border_changes=_boolean_rule(rules, "no_cell_border_changes"),
        no_worksheet_dimension_changes=_boolean_rule(
            rules, "no_worksheet_dimension_changes"
        ),
        ...
        no_formula_defined_xlm_registration_changes=_boolean_rule(
            rules, "no_formula_defined_xlm_registration_changes"
        ),
...
def evaluate_policy(report: DiffReport, policy: Policy) -> list[Finding]:
    ...
    if policy.no_external_relationship_changes:
        for finding in _rule_triggered(report, "FF063"):
            violations.append(
                Finding(
                    "FFP063",
                    "high",
    ...
    if policy.no_formula_defined_xlm_registration_changes:
        for finding in _rule_triggered(report, "FF068"):
            violations.append(
                Finding(
                    "FFP068",
                    "high",
                    (
                        "Policy forbids formula-defined XLM registration "
                        "changes."
                    ),
                    details=finding.details,
                )
            )
    if policy.no_formula_defined_xlm_evaluation_changes:
        for finding in _rule_triggered(report, "FF069"):
            violations.append(
                Finding(
                    "FFP069",
                    "high",
    ...
    if policy.no_chart_definition_changes:
        for finding in _rule_triggered(report, "FF030"):
            violations.append(
                Finding(
                    "FFP030",
                    "high",
    ...
    if policy.no_what_if_data_table_changes:
        for finding in _rule_triggered(report, "FF034"):
            violations.append(
                Finding(
                    "FFP034",
                    "high",
    ...
    if policy.no_filter_visibility_changes:
        for finding in _rule_triggered(report, "FF036"):
            violations.append(
                Finding(
                    "FFP036",
                    "high",
    ...
    if policy.no_table_style_control_changes:
        for finding in _rule_triggered(report, "FF061"):
            violations.append(
                Finding(
                    "FFP061",
                    "high",
                    "Policy forbids Excel Table Style control changes.",
                    details=finding.details,
                )
            )
    if policy.no_shared_workbook_revision_changes:
        for finding in _rule_triggered(report, "FF062"):
            violations.append(
                Finding(
                    "FFP062",
                    "high",
                    "Policy forbids legacy shared-workbook revision-history changes.",
                    details=finding.details,
                )
            )
    if policy.no_number_format_changes:
        for finding in _rule_triggered(report, "FF039"):
            violations.append(
                Finding(
                    "FFP039",
                    "high",
                    "Policy forbids cell number-format control changes.",
                    details=finding.details,
                )
            )
    if policy.no_cell_font_changes:
        for finding in _rule_triggered(report, "FF040"):
            violations.append(
                Finding(
                    "FFP040",
                    "high",
                    "Policy forbids cell-font control changes.",
                    details=finding.details,
                )
            )
    if policy.no_cell_fill_changes:
        for finding in _rule_triggered(report, "FF041"):
            violations.append(
                Finding(
                    "FFP041",
                    "high",
    ...
    if policy.no_workbook_theme_changes:
        for finding in _rule_triggered(report, "FF053"):
            violations.append(
                Finding(
                    "FFP053",
                    "high",
    ...
    if policy.no_cell_border_changes:
        for finding in _rule_triggered(report, "FF057"):
            violations.append(
                Finding(
                    "FFP057",
                    "high",
                    "Policy forbids cell-border control changes.",
                    details=finding.details,
                )
            )
    if policy.no_worksheet_dimension_changes:
        for finding in _rule_triggered(report, "FF058"):
            violations.append(
                Finding(
                    "FFP058",
                    "high",
                    "Policy forbids material worksheet-dimension control changes.",
                    details=finding.details,
                )
            )
    ...
    if policy.no_worksheet_print_layout_changes:
        for finding in _rule_triggered(report, "FF056"):
            violations.append(
                Finding(
                    "FFP056",
                    "high",
                    "Policy forbids material worksheet print-layout control changes.",
                    details=finding.details,
                )
            )
    ...
    if policy.no_rich_text_run_changes:
        for finding in _rule_triggered(report, "FF043"):
            violations.append(
                Finding(
                    "FFP043",
                    "high",
    ...
    if policy.no_legacy_comment_changes:
        for finding in _rule_triggered(report, "FF046"):
            violations.append(
                Finding(
                    "FFP046",
                    "high",
                    "Policy forbids legacy Excel Note and placeholder changes.",
                    details=finding.details,
                )
            )
    ...
    if policy.no_worksheet_drawing_shape_changes:
        for finding in _rule_triggered(report, "FF044"):
            violations.append(
                Finding(
                    "FFP044",
                    "high",
                    "Policy forbids Worksheet DrawingML shape, connector, or graphic-frame "
                    "control changes.",
                    details=finding.details,
                )
            )
