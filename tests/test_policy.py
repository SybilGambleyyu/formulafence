...
def test_policy_can_block_formula_external_action_changes(tmp_path) -> None:
...
def test_policy_can_block_formula_external_action_static_input_changes(tmp_path) -> None:
    baseline = make_formula_external_action_model(tmp_path / "baseline.xlsx")
    candidate = make_formula_external_action_model(tmp_path / "candidate.xlsx")
...
def test_policy_blocks_formula_defined_xlm_evaluation_changes(tmp_path) -> None:
    ...
    policy = parse_policy(
        {
            "version": 1,
            "rules": {"no_formula_defined_xlm_evaluation_changes": True},
        }
    )
...
def test_policy_blocks_formula_defined_xlm_get_cell_changes(tmp_path) -> None:
    baseline = make_formula_defined_xlm_get_cell_model(
        tmp_path / "baseline.xlsx"
    )
    candidate = make_formula_defined_xlm_get_cell_model(
        tmp_path / "candidate.xlsx"
    )
    ...
    policy = parse_policy(
        {
            "version": 1,
            "rules": {"no_formula_defined_xlm_get_cell_changes": True},
        }
    )
...
def test_policy_blocks_xlm_environment_information_changes(tmp_path) -> None:
...
def test_policy_can_block_worksheet_dimension_changes(tmp_path) -> None:
    ...
    report = compare_snapshots(load_snapshot(baseline), load_snapshot(candidate))
    policy = parse_policy(
        {"version": 1, "rules": {"no_worksheet_dimension_changes": True}}
    )
...
def test_policy_can_block_worksheet_display_control_changes(tmp_path) -> None:
    ...
    report = compare_snapshots(load_snapshot(baseline), load_snapshot(candidate))
    policy = parse_policy(
        {"version": 1, "rules": {"no_worksheet_display_control_changes": True}}
    )
...
def test_policy_can_block_rich_data_changes(tmp_path) -> None:
    baseline = make_rich_data_model(tmp_path / "baseline.xlsx")
    candidate = make_rich_data_model(tmp_path / "candidate.xlsx")
    change_rich_data_value(candidate)

    report = compare_snapshots(load_snapshot(baseline), load_snapshot(candidate))
    policy = parse_policy(
        {"version": 1, "rules": {"no_rich_data_changes": True}}
    )
...
def test_policy_can_block_custom_data_store_changes(tmp_path) -> None:
    ...
    change_custom_xml_data_store_value(candidate)

    report = compare_snapshots(load_snapshot(baseline), load_snapshot(candidate))
    policy = parse_policy(
        {"version": 1, "rules": {"no_custom_data_store_changes": True}}
    )
...
def test_policy_can_block_worksheet_smartart_diagram_changes(tmp_path) -> None:
...
def test_policy_can_block_worksheet_image_changes(tmp_path) -> None:
    baseline = make_worksheet_image_model(tmp_path / "baseline.xlsx")
    candidate = make_worksheet_image_model(tmp_path / "candidate.xlsx")
    change_worksheet_image_presentation(candidate)

    report = compare_snapshots(load_snapshot(baseline), load_snapshot(candidate))
    policy = parse_policy(
        {"version": 1, "rules": {"no_worksheet_image_changes": True}}
    )

    assert {finding.rule_id for finding in evaluate_policy(report, policy)} >= {"FFP059"}
